...
if (Test-Path -LiteralPath $binary) {
    ...
    if (Test-Path -LiteralPath $configFile) {
        ...
        if ($configItem.PSIsContainer) {
            ...
            if ($configText -match '# >>> ward (default permissions|permission profile) v[123] >>>|# ward:migrated-sandbox-(mode|workspace-write):v2|# ward:migrated-sandbox-mode:v3|default_permissions\s*=\s*"(ward|ward-baseline)"|\[permissions\.(ward|ward-baseline)\]') {
                $wardRefs = $true
            }
        }
    }
