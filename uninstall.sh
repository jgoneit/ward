...
if [ -e "$binary" ] || [ -L "$binary" ]; then
  ...
  printf 'removed %s\n' "$binary"
	...
	if [ -e "$config_file" ] && [ ! -f "$config_file" ]; then
		ward_refs=1
	elif [ -f "$config_file" ] && LC_ALL=C grep -Eq '# >>> ward (default permissions|permission profile) v[123] >>>|# ward:migrated-sandbox-(mode|workspace-write):v2|# ward:migrated-sandbox-mode:v3|default_permissions[[:space:]]*=[[:space:]]*"(ward|ward-baseline)"|\[permissions\.(ward|ward-baseline)\]' "$config_file"; then
		ward_refs=1
	fi
